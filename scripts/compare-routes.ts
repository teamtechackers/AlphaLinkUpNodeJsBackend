...
interface ComparisonResult {
  missing: Route[];
  mismatched: { node: NodeRoute; php: Route }[];
  extra: NodeRoute[];
  conflicts: { php: Route; postman: Route }[];
}
...
function compareRoutes(phpRoutes: Route[], nodeRoutes: NodeRoute[], postmanRoutes: Route[]): ComparisonResult {
...
function generateReport(comparison: ComparisonResult, phpRoutes: Route[], nodeRoutes: NodeRoute[], postmanRoutes: Route[]): string {
  ...
  try {
    ...
    nodeLines.forEach(line => {
      if (line.trim()) {
        ...
        nodeRoutes.push({ file, method, method, path, handler, line: parseInt(lineNum) });
      }
    });
    
    // Parse Postman routes
    const postmanContent = fs.readFileSync(postmanRoutesPath, 'utf8');
    const postmanRoutes: Route[] = [];
