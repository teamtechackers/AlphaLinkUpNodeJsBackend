...
class EventController {
  ...
  static async submitEvents(req, res) {
    try {
      ...
      // Safely resolve event owner user_id (avoid parseInt on encoded strings)
      const resolvedEventUserId = (eventUserId && !isNaN(parseInt(eventUserId))) ? parseInt(eventUserId) : decodedAdminId;

      if (!row_id || row_id === '') {
        // === ADD EVENT ===
        const insertData = {
          user_id: resolvedEventUserId,
  ...
  static async editEvents(req, res) {
    try {
      ...
      if (details.event_banner) {
        const baseUrl = process.env.BASE_URL || `${req.protocol}://${req.get('host')}`;
        details.event_banner = `${baseUrl}/uploads/events/${details.event_banner}`;
      }
