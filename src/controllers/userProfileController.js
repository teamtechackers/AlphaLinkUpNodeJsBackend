...
class UserProfileController {
  ...
  // API function - Save project details
  static async saveProjectDetails(req, res) {
    try {
      const { user_id, token, project_detail_id, project_name, description, project_description, project_url, startMonth, start_month, startYear, start_year, closedMonth, closed_month, closedYear, closed_year } = req.body;
      
      // Support both field name formats (Flutter and backend)
      const finalDescription = description || project_description;
      const finalStartMonth = startMonth || start_month;
      const finalStartYear = startYear || start_year;
      const finalClosedMonth = closedMonth || closed_month;
      const finalClosedYear = closedYear || closed_year;
      
      // Debug: Log received data
      console.log('Received project data:', {
        user_id, token, project_detail_id, project_name, 
        description, project_description, project_url, 
        startMonth, start_month, startYear, start_year,
        closedMonth, closed_month, closedYear, closed_year
      });
      console.log('Final values:', {
        finalDescription, finalStartMonth, finalStartYear, finalClosedMonth, finalClosedYear
      });
      ...
      if (!project_name || !finalDescription || !finalStartMonth || !finalStartYear) {
        console.log('Missing mandatory fields:', {
          project_name: !!project_name,
          finalDescription: !!finalDescription,
          finalStartMonth: !!finalStartMonth,
          finalStartYear: !!finalStartYear
        });
        return res.json({
          status: false,
          rcode: 500,
      ...
      // Use the month/year values directly
      ...
      if (finalProjectDetailId == 0) {
        ...
        const result = await query(
          `INSERT INTO user_project_details (user_id, project_name, description, project_url, start_month, start_year, closed_month, closed_year, project_logo) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [decodedUserId, project_name, finalDescription, project_url || '', finalStartMonth, finalStartYear, finalClosedMonth || '', finalClosedYear || '', projectLogo]
        );
        finalProjectDetailId = result.insertId;
      } else {
        // Update existing project detail
        const updateQuery = projectLogo ? 
          `UPDATE user_project_details 
           SET project_name = ?, description = ?, project_url = ?, start_month = ?, start_year = ?, closed_month = ?, closed_year = ?, project_logo = ?
           WHERE project_detail_id = ? AND user_id = ?` :
          `UPDATE user_project_details 
           SET project_name = ?, description = ?, project_url = ?, start_month = ?, start_year = ?, closed_month = ?, closed_year = ?
           WHERE project_detail_id = ? AND user_id = ?`;
        
        const updateParams = projectLogo ? 
          [project_name, finalDescription, project_url || '', finalStartMonth, finalStartYear, finalClosedMonth || '', finalClosedYear || '', projectLogo, finalProjectDetailId, decodedUserId] :
          [project_name, finalDescription, project_url || '', finalStartMonth, finalStartYear, finalClosedMonth || '', finalClosedYear || '', finalProjectDetailId, decodedUserId];
        ...
        if (updateResult.affectedRows === 0) {
          return res.json({
            status: false,
            rcode: 500,
            message: 'Project detail not found or access denied'
          });
        }
      }
      ...
      return res.json({
        ...
        user_id: user_id,
        unique_token: token,
        project_detail_id: finalProjectDetailId.toString(),
  ...
  // API function - Delete project detail
  static async deleteProjectDetail(req, res) {
    try {
      const { user_id, token, project_detail_id } = req.body;
      
      if (!user_id || !token || !project_detail_id) {
        return res.json({
          status: false,
          rcode: 500,
          message: 'Token Mismatch Exception'
      ...
      // Soft delete project detail
      const result = await query(
        'UPDATE user_project_details SET status = 0 WHERE project_detail_id = ? AND user_id = ?',
